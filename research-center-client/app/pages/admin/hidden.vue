<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Administração - Conteúdo Oculto</h1>
    ...
    <div v-else-if="activeTab === 'publications'">
      <div v-if="hiddenPublications.length === 0" class="text-center py-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500">Não existem publicações ocultas.</p>
      </div>
          ...
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">
                <span class="font-medium">Autores:</span> {{ pub.authors?.join(', ') || 'N/A' }}
              </p>
              <p class="text-sm text-gray-500">
                <span class="font-medium">Oculto por:</span> {{ pub.hiddenBy?.name || 'N/A' }}
              </p>
            </div>
    ...
    <div v-else-if="activeTab === 'comments'">
      <div v-if="hiddenComments.length === 0" class="text-center py-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500">Não existem comentários ocultos.</p>
      </div>
      <div v-else class="space-y-4">
          ...
          <div class="flex justify-between items-start">
            <div>
              ...
              <p class="text-sm text-gray-500 mt-2">
                <span class="font-medium">Autor:</span> {{ comment.author?.name || 'N/A' }}
              </p>
              <p class="text-sm text-gray-500">
                <span class="font-medium">Publicação:</span> {{ comment.publicationTitle || 'N/A' }}
              </p>
              <p class="text-sm text-gray-500">
                <span class="font-medium">Oculto por:</span> {{ comment.hiddenBy?.name || 'N/A' }}
              </p>
            </div>
    ...
    <div v-else-if="activeTab === 'tags'">
      <div v-if="hiddenTags.length === 0" class="text-center py-12 bg-gray-50 rounded-lg">
        <p class="text-gray-500">Não existem tags ocultas.</p>
      </div>
      <div v-else class="space-y-4">
...
const fetchData = async () => {
  ...
  try {
    ...
    if (e.response?.status === 403) {
      error.value = 'Não tem permissões para aceder a esta página.'
    } else {
      error.value = 'Erro ao carregar conteúdo oculto.'
    }
  } finally {
    loading.value = false
  }
