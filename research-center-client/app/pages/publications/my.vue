  ...
  <div class="container mx-auto px-4 py-6">
    <div class="bg-white shadow rounded-lg p-6">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="text-2xl font-bold text-slate-800">As Minhas Publicações</h2>
          <p class="text-sm text-slate-500">Todas as publicações submetidas por mim (incluindo não visíveis)</p>
        </div>
        ...
        <div v-else class="space-y-4">
          <div v-for="p in items" :key="p.id" class="border rounded-lg p-4 hover:shadow-lg transition">
            ...
            <div class="flex items-center gap-4 mt-3 text-sm text-slate-600">
              ...
              <div>👁 {{ p.viewsCount || 0 }} visualizações</div>
            </div>
          </div>
        </div>

        <div v-if="items.length > 0" class="flex justify-between items-center mt-6">
          <div class="text-sm text-slate-600">{{ items.length }} de {{ totalElements }} publicações</div>
